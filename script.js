...
if (location.pathname.endsWith("index.html") || location.pathname.endsWith("/")) {
  ...
  f.addEventListener("submit", e => {
    ...
    const name = document.getElementById("name").value.trim(),
      ...
      qual = document.getElementById("qual").value.trim(),
      pass = document.getElementById("password").value;

    if (!name || !email || !qual || !pass) {
    ...
    msg.innerHTML =
      'Signup successful! Your Access Code: <strong style="color:#0b74a6">' + code + "</strong>";
...
if (location.pathname.endsWith("login.html")) {
  ...
  f.addEventListener("submit", e => {
    ...
    if (code) {
      ...
      sessionStorage.setItem("govt_current", JSON.stringify({ email }));
    ...
    saveUserObj(user);
    msg.innerHTML =
      'Logged in. New Access Code: <strong style="color:#0b74a6">' + newCode + "</strong>";
    sessionStorage.setItem("govt_current", JSON.stringify({ email }));
    setTimeout(() => (location.href = "select.html"), 1200);
  });
...
if (location.pathname.endsWith("select.html")) {
  const cur = JSON.parse(sessionStorage.getItem("govt_current") || "null");
  ...
  document.getElementById("logout").addEventListener("click", () => {
    sessionStorage.removeItem("govt_current");
